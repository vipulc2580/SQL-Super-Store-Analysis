...
10. **What is the average sales amount per transaction?**
	```sql
	    select ceil(avg(totalamount)) as Average_Sales from sales;
	```
       
11. **How many sales transactions were made by each sales representative?**
	```sql
...
25. **Which suppliers products have the highest total sales by quantity?**
	```sql
